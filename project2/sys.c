...
asmlinkage long sys_cs1550_down(struct cs1550_sem *sem) 
{
	...
	*/
    ...
    if(sem -> value <= 0){ //return condition
	...
	if head not null
		get tail
		create new node
		append tot tail -> next
		sleep
    */
    //queue process
	struct Node *p;
    p = (struct Node*)kmalloc(sizeof(struct Node), GFP_KERNEL);
    p -> process = current;
    p -> next = NULL;
    if(sem -> head == NULL){
    	sem -> head = p;
	...
	set_current_state(TASK_INTERRUPTIBLE); //sleep part 1
	//unlock
	spin_unlock(&sem_lock);
	schedule(); //sleep part 2
    return 0;
...
asmlinkage long sys_cs1550_up(struct cs1550_sem *sem) 
{
    ...
    */
    ...
    temp_task = temp_node -> process;


    if(sem -> head == sem -> tail){ //reset head and tail
    	sem -> head = NULL;
    	sem -> tail == NULL;
    }
    else{
    	sem -> head = temp_node -> next;
    }

    wake_up_process(temp_task);
